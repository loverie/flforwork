...
class ClientsGroup(object):
    ...
    def dataSetBalanceAllocation(self):
        ...
        # test_label = torch.tensor(cifar10DataSet.test_label).argmax(dim=1)
        test_label = torch.argmax(torch.tensor(cifar10DataSet.test_label), dim=1)

        if self.central_data is None:
            order = np.arange(test_data.shape[0])
            np.random.shuffle(order)
            # CIFAR-10 数据集的中央数据集加载
