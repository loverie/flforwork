...
from Models import Cifar10_CNN, Mnist_CNN, ResNet20
...
def model2vector(model):
    ...
    for key, var in model.items():
        ...
        if key.split('.')[-1] in ['num_batches_tracked', 'running_mean', 'running_var']:
            continue
        # 确保 var 是浮点数类型
        var_tensor = var.float()
        nplist = var_tensor.cpu().numpy()
        nplist = nplist.ravel()
...
def get_weight(update, model):
    for key, var in update.items():
        # 确保 update[key] 和 model[key] 都是浮点数类型
        var = var.float()  # 将更新的参数转换为浮点数类型
        model_tensor = model[key].float()  # 将模型参数转换为浮点数类型
        # 执行减法操作，得到更新量
        var -= model_tensor
    return update
...
def detection1(local_updates1 ,update_vectors1, global_vector, clients_in_comm, trust_scores,update_weights=None):
    ...
    # 过滤用户分数低于 3.4的用户
    high_confidence_users = np.where(user_confidence_scores >= 1)[0]

    # 选择更新
    selected_updates = [local_updates1[i] for i in high_confidence_users]

    # 计算权重
    selected_user_scores = user_confidence_scores[high_confidence_users]
...
parser.add_argument('-mn', '--model_name', type=str, default='ResNet20', help='the model to train(ResNet20,Cifar10_CNN)')
...
parser.add_argument('-atp', '--attack_type', type=int, default=5, help='the types to attack')
parser.add_argument('-att', '--attack_turn', type=int, default=1, help='the turns to attack')
parser.add_argument('-atn', '--attack_num', type=int, default=10, help='the num to attack')
...
if __name__ == "__main__":
    ...
    # print(attack_rounds)
    #minist/cifar10
    myClients = ClientsGroup('cifar10', args['IID'], args['num_of_clients'], dev,attack_rounds=attack_rounds)
    tempGroup=myClients
    testDataLoader = myClients.test_data_loader
    ...
    for i in range(args['num_comm']):
        ...
        # print('每一轮global_updates',global_updates)
        ...
        for client_name in tqdm(clients_in_comm):
            ...
            # print('每一轮local',local_array)
            # print('每一轮变化',model2vector(temp_update))
        ...
        for var in aggregated_update:
            # 确保 aggregated_update 中的张量是 Float 类型
            aggregated_update[var] = aggregated_update[var].float()
        ...
        # print('训练之后：',model2vector(global_parameters))
        ...
        with torch.no_grad():
            if (i + 1) % args['val_freq'] == 0:
                ...
                for data, label in testDataLoader:
                    ...
                    # pred_classes = label.argmax(dim=1)  # 这将返回每个样本最可能的类别索引
    ...
    plt.savefig(os.path.join(res_dir, '1111持续.png'))