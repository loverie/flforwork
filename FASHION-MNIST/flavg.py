...
parser.add_argument('-sf', '--save_freq', type=int, default=10000, help='global model save frequency(of communication)')
...
parser.add_argument('-atn', '--attack_num', type=int, default=60, help='the num of clients to attack')
...
if __name__ == "__main__":
    ...
    for i in range(args['num_comm']):
        ...
        if(args['attack_type']==2):
            if (i + 1) in myClients.attack_rounds:
                for client in clients_in_comm:
                    myClients.flag_group_attack(args['attack_num'], client=client,  groups=groups,  group_size=args['group_size'],  groupFlag=groupFlag,  max_group_id=max_group_id,  clients_in_comm=clients_in_comm)
        ...
        for client in tqdm(clients_in_comm):
            ...
            if sum_parameters is None:
                sum_parameters = {key: torch.zeros_like(var) for key, var in global_parameters.items()}
            for var in sum_parameters:
                sum_parameters[var] += local_parameters[var]
        ...
        for var in global_parameters:
            #除以所以客户端的信任得分总和   聚合
            global_parameters[var] += sum_parameters[var] / (10)
    ...
    plt.savefig(os.path.join(res_dir, 'flavg0.05.png'))
