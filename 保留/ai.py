...
def detection1(local_updates1 ,update_vectors1, global_vector, clients_in_comm, trust_scores,update_weights=None):
    ...
    # 选择top N更新
    N = min(len(high_confidence_users), len(update_vectors))
    selected_user_indices = np.argsort(-user_confidence_scores[high_confidence_users])[:N]
    selected_updates = [local_updates1[i] for i in high_confidence_users[selected_user_indices]]

    # # 选择top N更新
    # N = min(len(valid_users), len(high_confidence_users))
    # selected_user_indices = np.argsort(-user_confidence_scores[valid_users])[:N]
    # selected_updates = [local_updates1[i] for i in valid_users[selected_user_indices]]
    # 计算权重
    selected_user_scores = user_confidence_scores[high_confidence_users[selected_user_indices]]
    selected_user_weights = selected_user_scores / np.sum(selected_user_scores)

    # # 计算权重
    # selected_user_scores = user_confidence_scores[valid_users[selected_user_indices]]
    # selected_user_weights = selected_user_scores / np.sum(selected_user_scores)
