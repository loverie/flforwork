...
logging.basicConfig(filename='logs/DSCN5.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def detection1(local_updates1 ,local_updatesarr,update_vectors1, global_vector, clients_in_comm, trust_scores,global_change, update_weights=None):
    ...
    if(clusterIndexForTrust != -1):
        update_vectors_cluster1 = update_vectors[labels == clusterIndexForTrust]
        centroid = update_vectors_cluster1.mean(axis=0)
...
parser.add_argument('-nc', '--num_of_clients', type=int, default=50, help='number of the clients')
