...
logging.basicConfig(filename='logs/40用户60恶意0.05.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def cos(a, b):
    ...
    res = (np.dot(a, b) + 1e-9) / (np.linalg.norm(a) + 1e-9) / (np.linalg.norm(b) + 1e-9)
    '''relu'''
    if res < 0:
        res = 0
    return res
...
def detection1(local_updates1 ,update_vectors1, global_vector, clients_in_comm, trust_scores,update_weights=None):
    ...
    spectral_clustering = SpectralClustering(n_clusters=best_k, affinity='nearest_neighbors', random_state=42)
    spectral_clustering.fit(update_vectors)
    labels = spectral_clustering.labels_
    # centroids = kmeans_best.cluster_centers_
    # for label in range(labels):
    #     update_vectors_cluster = update_vectors[labels == label]
    ...
    for i in range(best_k):
        ...
        if update_vectors_cluster.size > 0:
            temp = copy.deepcopy(update_vectors_cluster.mean(axis=0))
            centroid.append(temp)
            sim = cos(centroid[i], global_vector)
            trust_score = 1 + sim
    ...
    for idx, centroid in enumerate(centroids):
        ...
        for user_idx in cluster_indices:
            # print(user_idx)
            # distance = cos(update_vectors[user_idx], global_vector)
            distance = cos(update_vectors[user_idx], global_vector)
            ...
            # user_confidence_scores[user_idx] = ((0.125*trust_score_value) + (distance*0.875)) * trust_scores_temp[idx][0]
    ...
    # 过滤用户分数低于 3.4的用户
    high_confidence_users = np.where(user_confidence_scores > 3.4)[0]
    ...
    for i in range(args['num_comm']):
        ...
        with torch.no_grad():
            if (i + 1) % args['val_freq'] == 0:
                ...
                logging.info(f"K-Fedtrust round {i+1} completed . Validation accuracy: {accuracy:.4f}")
