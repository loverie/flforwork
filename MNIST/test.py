...
logging.basicConfig(filename='logs/K-Fedtrust3.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def detection1(local_updates1 ,local_updatesarr,update_vectors1, global_vector, clients_in_comm, trust_scores,global_change, update_weights=None):
    ...
    update_vectors = []
    update_vectors = np.array(local_updatesarr)
    for i in range(35):  # 假设前35个用户的更新向量
        update_vectors[i] *= 100  # 放大前35个用户的更新向量，以便在聚类时更容易区分
