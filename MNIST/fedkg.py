'''
1.标签翻转攻击本身应该就比较弱
2.目前分簇之后,其有概率错分(对于非目标来说,错分概率较小),导致簇中心和用户更新计算余弦相似度,本身就不会小于0,目前的效果好主要靠(没有了检测就相当于fedavg,而fedavg本身对标签翻转也有抵抗作用,准确率很高)
3.iid数据本身就对标签翻转攻击有一些抵抗作用(guess)
'''
import os
import copy
import argparse
...
def detection1(local_updates1 ,update_vectors1, global_vector, clients_in_comm, trust_scores,update_weights=None):
    ...
    # 过滤用户分数低于 3.4的用户
    high_confidence_users = np.where(user_confidence_scores > 3.7)[0]
