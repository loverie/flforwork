...
def norm_clip(nparr1, nparr2):
    ...
    norm2 = np.linalg.norm(nparr2, ord=None, axis=None, keepdims=False)
    if norm2 == 0:
        # 避免除以零，可以返回一个很大的数值或者特定值
        return float('inf')  # 或者根据情况选择其他合适的值
    return vnum / norm2 + 1e-9
...
parser.add_argument('-atn', '--attack_num', type=int, default=50, help='the num of clients to attack')
...
if __name__ == "__main__":
    ...
    for i in range(args['num_comm']):
        ...
        clients_in_comm1 = ['client{}'.format(i) for i in range(args['attack_num'],)]
        # ----------------标签反转攻击-----------------
        ...
        FLTrustCentralNorm = myClients.centralTrain(args['epoch'], args['batchsize'], net,
                                                    loss_func, opti, global_parameters,args['activate_backdoor'])
        '''get the update weight'''
        ...
        for client in tqdm(clients_in_comm):
            ...
            if sum_parameters is None:
                ...
                for var in sum_parameters:
                    if not np.isnan(client_score) and not np.isinf(client_score) and \
       not np.isnan(client_clipped_value) and not np.isinf(client_clipped_value):
                        sum_parameters[var] += client_score * client_clipped_value * local_parameters[var]
    ...
    plt.savefig(os.path.join(res_dir, 'FLTrust5011持续.png'))
